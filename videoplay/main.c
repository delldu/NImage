...
int play_video_with_image(char *input_filename, int start, int n, char *output_dir)
{
	IMAGE *image;
	VIDEO *video;
	...
	while (n > 0 && ! video_eof(video)) {
		...
		snprintf(output_file_name, sizeof(output_file_name), "%s/%06d.png", output_dir, start++);
...
int play_video_with_tensor(char *input_filename, int start, int n, char *output_dir)
{
	IMAGE *image;
	VIDEO *video;
	TENSOR *tensor;
...
void help(char *cmd)
{
	...
	printf("    -o, --output <dir>           Output diretory (default: output).\n");
...
int main(int argc, char **argv)
{
	...
	while ((optc = getopt_long(argc, argv, "h s: n: o: t", long_opts, &option_index)) != EOF) {
		switch (optc) {
		case 's':
			start = atoi(optarg);
			break;
	...
	if (argc > optind) {
		return (tensor == 0)?play_video_with_image(argv[optind], start, number, output_dir)
				: play_video_with_tensor(argv[optind], start, number, output_dir);
	} else {
