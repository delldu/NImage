...
void help(char *cmd) {
  printf("This is a simple example for nimage, client send image to server and "
         "server echo back\n");
...
int main(int argc, char **argv) {
  ...
  struct option long_opts[] = {{"help", 0, 0, 'h'},   {"endpoint", 1, 0, 'e'},
                               {"server", 0, 0, 's'}, {"client", 1, 0, 'c'},
                               {"output", 1, 0, 'o'}, {0, 0, 0, 0}};

  if (argc <= 1)
    help(argv[0]);

  while ((optc = getopt_long(argc, argv, "h e: s c: o:", long_opts,
                             &option_index)) != EOF) {
    switch (optc) {
    case 'e':
      endpoint = optarg;
