...
static int __video_probe(char* filename, VIDEO* v)
{
    ...
    snprintf(cmd, sizeof(cmd) - 1, "ffprobe -show_streams %s 2>/dev/null",
        filename);
...
VIDEO* __camera_open(char* filename, int start)
{
    ...
    for (i = 0; i < VIDEO_BUFFER_NUMS; i++) {
        v->frames[i] = frame_create(v4l2fmt.fmt.pix.pixelformat,
    ...
    for (i = 0; i < VIDEO_BUFFER_NUMS; i++) {
        ...
        v->_frame_buffer[i].startmap = mmap(NULL /* start anywhere */, buf.length,
            PROT_READ | PROT_WRITE /* required */,
            MAP_SHARED /* recommended */, fileno(fp), buf.m.offset);
...
VIDEO* __file_open(char* filename, int start)
{
    ...
    // snprintf(cmdline, sizeof(cmdline) - 1, "ffmpeg -i %s -f rawvideo -
    // 2>/dev/null", filename);
    snprintf(
...
void video_info(VIDEO* v)
{
    ...
    syslog_info("Total frames: %d, frame size: %d x %d, format: %c%c%c%c",
        v->frame_numbers, v->width, v->height, GET_FOURCC1(v->format),
        GET_FOURCC2(v->format), GET_FOURCC3(v->format),
...
int video_eof(VIDEO* v) { return (!video_valid(v) || feof(v->_fp)); }
...
int video_encode(char* input_dir, char* output_file)
{
    ...
    // ffmpeg -i "$1" -vcodec avc -c:v h264 -preset slow -crf 11 -vf
    // format=yuv420p -y "$2" snprintf(cmdline, sizeof(cmdline) - 1, 	"ffmpeg
    // -i %s/%%6d.png -vcodec avc -c:v h264 -preset slow -crf 11 -vf
    // format=yuv420p -y %s", 	input_dir, output_file);

    snprintf(cmdline, sizeof(cmdline) - 1,
        "ffmpeg -i %s/%%6d.png -vcodec png -y %s", input_dir, output_file);
    if ((fp = popen(cmdline, "r")) == NULL) {
        syslog_error("Open %s.", cmdline);
        return RET_ERROR;
    }
...
int video_decode(char* input_file, char* output_dir)
{
    ...
    snprintf(cmdline, sizeof(cmdline) - 1, "ffmpeg -i %s -q:v 2 -y %s/%%6d.png",
