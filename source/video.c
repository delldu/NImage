...
static VIDEO *__yuv420_open(char *filename, int width, int height) {
  ...
  // Allocate memory
  for (i = 0; i < VIDEO_BUFFER_NUMS; i++) {
    v->_frame_buffer[i].startmap =
        (BYTE *)calloc((size_t)v->frame_size, sizeof(BYTE));
...
VIDEO *__file_open(char *filename, int start) {
  ...
  snprintf(cmdline, sizeof(cmdline) - 1,
           "ffmpeg -i %s -f rawvideo - 2>/dev/null", filename);
...
int video_valid(VIDEO *v) {
  if (!v || v->height < 1 || v->width < 1 || v->frame_size < 1 ||
      v->magic != VIDEO_MAGIC)
    return 0;
