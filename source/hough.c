...
LINES *line_set() { return &__global_line_set; }
...
#define MAX_ANGLES 180
  ...
  rmax =
      (int)sqrt(image->height * image->height + image->width * image->width) +
      1;
  ...
  matrix_foreach(hough, i, j) {
    if ((int)hough->me[i][j] >= threshold) {
      line_put((int)begrow->me[i][j], (int)begcol->me[i][j],
               (int)endrow->me[i][j], (int)endcol->me[i][j]);
      if (debug)
        image_drawline(image, (int)begrow->me[i][j], (int)begcol->me[i][j],
                       (int)endrow->me[i][j], (int)endcol->me[i][j], 0x00ffff);
    }
  }
