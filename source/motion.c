...
RECTS* rect_set() { return &__global_rect_set; }
...
int has_weight(int w) { return (w >= object_min_points) ? 1 : 0; }
...
int motion_updatebg(IMAGE* A, IMAGE* B, IMAGE* C, IMAGE* bg)
{
    ...
    {
        k = (A->ie[i][j].r - B->ie[i][j].r) * (A->ie[i][j].r - B->ie[i][j].r) + (A->ie[i][j].g - B->ie[i][j].g) * (A->ie[i][j].g - B->ie[i][j].g) + (A->ie[i][j].b - B->ie[i][j].b) * (A->ie[i][j].b - B->ie[i][j].b);
        a = (k >= threshold) ? 1 : 0;
        k = (C->ie[i][j].r - B->ie[i][j].r) * (C->ie[i][j].r - B->ie[i][j].r) + (C->ie[i][j].g - B->ie[i][j].g) * (C->ie[i][j].g - B->ie[i][j].g) + (C->ie[i][j].b - B->ie[i][j].b) * (C->ie[i][j].b - B->ie[i][j].b);
        b = (k >= threshold) ? 1 : 0;
...
int motion_detect(IMAGE* fg, IMAGE* bg, int debug)
{
    ...
    {
        k = (fg->ie[i][j].r - bg->ie[i][j].r) * (fg->ie[i][j].r - bg->ie[i][j].r) + (fg->ie[i][j].g - bg->ie[i][j].g) * (fg->ie[i][j].g - bg->ie[i][j].g) + (fg->ie[i][j].b - bg->ie[i][j].b) * (fg->ie[i][j].b - bg->ie[i][j].b);
        if (k >= threshold)
            diffmat->me[i][j] = 1;
    }
    mrs->count = 0;
