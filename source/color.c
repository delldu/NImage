...
extern int color_prmgain(IMAGE *img, float *r_gain, float *g_gain,
                         float *b_gain);
...
void color_rgb2lab(BYTE R, BYTE G, BYTE B, float *L, float *a, float *b) {
  ...
  r0 = (r0 > 0.04045f) ? powf((r0 + 0.055f) / 1.055f, 2.4f) : r0 / 12.92f;
  g0 = (g0 > 0.04045f) ? powf((g0 + 0.055f) / 1.055f, 2.4f) : g0 / 12.92f;
  b0 = (b0 > 0.04045f) ? powf((b0 + 0.055f) / 1.055f, 2.4f) : b0 / 12.92f;

  x = (r0 * 0.412453f + g0 * 0.357580f + b0 * 0.180423f) / 0.95047f;
  y = (r0 * 0.212671f + g0 * 0.715160f + b0 * 0.072169f) / 1.00000f;
  z = (r0 * 0.019334f + g0 * 0.119193f + b0 * 0.950227f) / 1.08883f;

  x = (x > 0.008856f) ? powf(x, 0.333333f) : (7.787f * x) + 16.f / 116.f;
  y = (y > 0.008856f) ? powf(y, 0.333333f) : (7.787f * y) + 16.f / 116.f;
  z = (z > 0.008856f) ? powf(z, 0.333333f) : (7.787f * z) + 16.f / 116.f;

  *L = CLAMP(116.f * y - 16.f, 0.f, 100.f);
  *a = CLAMP(500.f * (x - y), -110.f, 110.f);
  *b = CLAMP(200.f * (y - z), -110.f, 110.f);
}
...
void color_lab2rgb(float L, float a, float b, BYTE *R, BYTE *G, BYTE *B) {
  ...
  // x * x * x > 0.008856 <==> x > 0.2068966
  x = 0.95047f * ((x > 0.2068966f) ? x * x * x : (x - 16.f / 116.f) / 7.787f);
  y = 1.00000f * ((y > 0.2068966f) ? y * y * y : (y - 16.f / 116.f) / 7.787f);
  z = 1.08883f * ((z > 0.2068966f) ? z * z * z : (z - 16.f / 116.f) / 7.787f);

  r0 = x * 3.24048134f - y * 1.53715152f - z * 0.49853633f;
  g0 = -x * 0.96925495f + y * 1.87599000f + z * 0.04155593f;
  b0 = x * 0.05564664f - y * 0.20404134f + z * 1.05731107f;
  ...
  // 0.416666 == 1.f/2.4f
  r0 =
      (r0 > 0.0031308f) ? (1.055f * powf(r0, 0.416666f) - 0.055f) : 12.92f * r0;
  g0 =
      (g0 > 0.0031308f) ? (1.055f * powf(g0, 0.416666f) - 0.055f) : 12.92f * g0;
  b0 =
      (b0 > 0.0031308f) ? (1.055f * powf(b0, 0.416666f) - 0.055f) : 12.92f * b0;
...
int color_cluster(IMAGE *image, int num) {
  ...
  for (c = 0; c < num && c < ARRAY_SIZE(image->KColors); c++) {
    image->KColors[c] = RGB_INT((BYTE)ccmat->me[c][0], (BYTE)ccmat->me[c][1],
                                (BYTE)ccmat->me[c][2]);
    image->KCounts[c] =
        (int)ccmat->me[c][3]; // r, g, b, count, orig class, sorted class no
  }
...
int *color_count(IMAGE *image, int rows, int cols, int levs) {
  ...
  if (levs < 1 || rows < 1 || cols < 1) {
    syslog_error("rows(%d), cols(%d), levs(%d) must be greater 1", rows, cols,
                 levs);
    return NULL;
  }
...
int color_picker() {
  static int color_set[9] = {0x00ff00, 0x0000ff, 0x00ffff, 0xff00ff, 0xffff00,
                             0x7f0000, 0x007f00, 0x00ff7f, 0x007f7f};
...
int color_rect_gwmgain(IMAGE *img, RECT *rect, float *r_gain, float *g_gain,
                       float *b_gain) {
  int i, j;
...
int color_rect_prmgain(IMAGE *img, RECT *rect, float *r_gain, float *g_gain,
                       float *b_gain) {
  int i, j, k, threshold;
  ...
  rect_foreach(rect, i, j) {
    count[img->ie[i + rect->r][j + rect->c].r +
