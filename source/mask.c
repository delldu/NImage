...
inline void max_statck_reset() { max_stack.count = 0; }
...
inline int max_stack_empty() { return max_stack.count < 1; }

inline void min_statck_reset() { min_stack.count = 0; }
...
inline int min_stack_empty() { return min_stack.count < 1; }
        ...
        while (!max_stack_empty()) {                                                                                                                 \
            ...
            for (i2 = start_row; i2 <= stop_row; i2++) {                                                                                             \
                for (j2 = start_col; j2 <= stop_col; j2++) {                                                                                         \
                    if (image->ie[i2][j2].a == image->ie[row][col].a && RGB_INT(image->ie[i2][j2].r, image->ie[i2][j2].g, image->ie[i2][j2].b) == 0) \
...
int color_instance(MASK* image, int KRadius)
{
    ...
    image_foreach(image, i, j) image->ie[i][j].r = image->ie[i][j].g = image->ie[i][j].b = 0; // Clear all labels
