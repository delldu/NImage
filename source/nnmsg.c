...
int tensor_send(int socket, int msgcode, TENSOR *tensor) {
  ...
  while (left > 0) {
    ...
    if (n < 0) {
      syslog_error("nn_send: error code = %d, message = %s", nn_errno(),
...
TENSOR *tensor_recv_timeout(int socket, int timeout, int *msgcode) {
  ...
  *msgcode = 0;
  ...
  if (size < 0) {
    syslog_error("nn_recv: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return NULL;
  }
...
int socket_readable(int socket, int timeout) {
  ...
  if (timeout > 0) {
    ...
    if (rc == -1) {
      // syslog_info("nn_poll: error code = %d, message = %s", nn_errno(),
      // nn_strerror(nn_errno()));
      return 0;
    }
...
int server_open(char *endpoint) {
  ...
  if ((socket = nn_socket(AF_SP, NN_REP)) < 0) {
    syslog_error("nn_socket: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  }

  if (nn_setsockopt(socket, NN_SOL_SOCKET, NN_SNDBUF, &buffer_size,
                    sizeof(buffer_size)) < 0) {
    syslog_error("nn_setsockopt: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  }

  if (nn_setsockopt(socket, NN_SOL_SOCKET, NN_RCVBUF, &buffer_size,
                    sizeof(buffer_size)) < 0) {
    syslog_error("nn_setsockopt: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  ...
  if (nn_setsockopt(socket, NN_SOL_SOCKET, NN_RCVMAXSIZE, &max_recv_size,
                    sizeof(max_recv_size)) < 0) {
    syslog_error("nn_setsockopt: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  }
  ...
  if (nn_bind(socket, endpoint) < 0) {
    syslog_error("nn_bind: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  }
...
int client_open(char *endpoint) {
  ...
  if (nn_setsockopt(socket, NN_SOL_SOCKET, NN_RCVTIMEO, &recv_timeout,
  ...
  if (nn_connect(socket, endpoint) < 0) {
    syslog_error("nn_connect: error code = %d, message = %s", nn_errno(),
                 nn_strerror(nn_errno()));
    return -1;
  }

  if (service_avaible(socket) != RET_OK) {
    syslog_error("Sorry, We could not got hello response from server in 5 "
                 "seconds, so stop !");
    client_close(socket);
    return -1;
  }
...
void client_close(int socket) { nn_close(socket); }

void server_close(int socket) { nn_shutdown(socket, 0); }
