...
int image_blend(IMAGE *src, IMAGE *mask, IMAGE *dst, int top, int left,
  ...
  image_foreach(mask, i, j) {
    ...
    if (__mask_border(mask, i, j)) {
      d = MASK_ALPHA * src->ie[i][j].r +
          (1 - MASK_ALPHA) * dst->ie[i + top][j + left].r;
      dst->ie[i + top][j + left].r = (BYTE)d;

      d = MASK_ALPHA * src->ie[i][j].g +
          (1 - MASK_ALPHA) * dst->ie[i + top][j + left].g;
      dst->ie[i + top][j + left].g = (BYTE)d;

      d = MASK_ALPHA * src->ie[i][j].b +
          (1 - MASK_ALPHA) * dst->ie[i + top][j + left].b;
...
int *image_seampath(IMAGE *image_a, RECT *rect_a, IMAGE *image_b, RECT *rect_b,
  ...
  case 0: // A, Center = (h, w)
    matrix_foreach(mat, i, j) {
      ...
      r2 = 1.0f;
      a = (image_a->ie[i + rect_a->r][j + rect_a->c].r -
           image_b->ie[i + rect_b->r][j + rect_b->c].r);
      b = (image_a->ie[i + rect_a->r][j + rect_a->c].g -
           image_b->ie[i + rect_b->r][j + rect_b->c].g);
      c = (image_a->ie[i + rect_a->r][j + rect_a->c].b -
           image_b->ie[i + rect_b->r][j + rect_b->c].b);
      x2 = a * a + b * b + c * c;
      mat->me[i][j] = r2 * x2;
    }
    break;
