...
extern int matrix_fast_guided_filter(MATRIX *P, MATRIX *I, int radius,
                                     float eps, int scale);
extern int matrix_guided_filter(MATRIX *mat, MATRIX *guidance, int radius,
                                float eps);
...
// P --, I -- guidance
int __guided_means(MATRIX *P, MATRIX *I, int radius, float eps, MATRIX *mean_a,
                   MATRIX *mean_b) {
  int i, j;
...
// P --, I -- guidance
int matrix_fast_guided_filter(MATRIX *P, MATRIX *I, int radius, float eps,
                              int scale) {
  int ret;
  ...
  ret =
      __guided_means(small_P, small_I, radius, eps, small_mean_a, small_mean_b);
  if (ret == RET_OK) {
...
int image_gauss_filter(IMAGE *image, float sigma) {
  ...
  if (image->format == IMAGE_GRAY) {
    ...
    matrix_destroy(mat);
    image_foreach(image, i, j) image->ie[i][j].g = image->ie[i][j].b =
        image->ie[i][j].r;
  } else {
    mat = image_getplane(image, 'R');
    check_matrix(mat);
...
int image_guided_filter(IMAGE *img, IMAGE *guidance, int radius, float eps,
                        int scale, int debug) {
