...
void image_membind(IMAGE *img, WORD h, WORD w) {
  ...
  img->ie = (RGBA_8888 **)(base + sizeof(IMAGE) +
                           (h * w) * sizeof(RGBA_8888)); // Skip head and data
  for (i = 0; i < h; i++)
    ...
    for (i = 0; i < (rect)->h; i++) {
      ...
      for (j = 0; j < (rect)->w; j++) {
        ...
        y2 = (66 * (img->ie[i + rect->r][j + rect->c].r) +
              ...
              25 * (img->ie[i + rect->r][j + rect->c].b) + 128) /
                 256 +
             16;
...
int image_valid(IMAGE *img) {
  return (!img || img->height < 1 || img->width < 1 || !img->ie ||
          img->magic != IMAGE_MAGIC)
             ? 0
             : 1;
}
...
int image_outdoor(IMAGE *img, int i, int di, int j, int dj) {
  return (i + di < 0 || i + di >= img->height || j + dj < 0 ||
          j + dj >= img->width);
}
...
static IMAGE *image_loadjpeg(char *fname) {
  ...
  lineBuf = cinfo.mem->alloc_sarray((j_common_ptr)&cinfo, JPOOL_IMAGE,
                                    (cinfo.output_width * bytes_per_pixel), 1);
...
IMAGE *image_loadpng(char *fname) {
  ...
  if ((png_pixels =
           (png_byte *)malloc(row_bytes * height * sizeof(png_byte))) == NULL) {
    syslog_error("Allocate memeory.");
    png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
    return NULL;
  }
  ...
  // Sorted chanels
  if (channels < 2) {
    image_foreach(image, row, col) image->ie[row][col].g =
        image->ie[row][col].r;
    image_foreach(image, row, col) image->ie[row][col].b =
        image->ie[row][col].r;
  }
  if (channels < 3) {
    image_foreach(image, row, col) image->ie[row][col].b =
...
int image_drawline(IMAGE *img, int r1, int c1, int r2, int c2, int color) {
  int i, j;
  BYTE r, g, b;
  int temp, adj_up, adj_down, error_term, x_advance, x_delta, y_delta,
      whole_step, initial_pixel_count, final_pixel_count, run_length;
  ...
  /* Special-case horizontal, vertical, and diagonal lines, for speed and to
   * avoid nasty boundary conditions and division by 0. */
...
int image_paste(IMAGE *img, int r, int c, IMAGE *small, float alpha) {
  ...
  for (i = 0; i < y2 - r; i++) {
    for (j = 0; j < x2 - c; j++) {
      img->ie[i + r][j + c].r =
          (BYTE)(alpha * img->ie[i + r][j + c].r +
                 (1.0f - alpha) * small->ie[i + offy][j + offx].r);
      img->ie[i + r][j + c].g =
          (BYTE)(alpha * img->ie[i + r][j + c].g +
                 (1.0f - alpha) * small->ie[i + offy][j + offx].g);
      img->ie[i + r][j + c].b =
          (BYTE)(alpha * img->ie[i + r][j + c].b +
                 (1.0f - alpha) * small->ie[i + offy][j + offx].b);
    }
  }
...
int image_rect_paste(IMAGE *bigimg, RECT *bigrect, IMAGE *smallimg,
                     RECT *smallrect) {
  int i, j;
  check_image(bigimg);
  ...
  for (i = 0; i < smallrect->h; i++) {
    for (j = 0; j < smallrect->w; j++) {
      bigimg->ie[i + bigrect->r][j + bigrect->c].r =
          smallimg->ie[i + smallrect->r][j + smallrect->c].r;
      bigimg->ie[i + bigrect->r][j + bigrect->c].g =
          smallimg->ie[i + smallrect->r][j + smallrect->c].g;
      bigimg->ie[i + bigrect->r][j + bigrect->c].b =
          smallimg->ie[i + smallrect->r][j + smallrect->c].b;
    }
  }
  return RET_OK;
}
...
int image_rect_statistics(IMAGE *img, RECT *rect, char orgb, float *avg,
                          float *stdv) {
  BYTE n;
  int i, j;
  float davg, dstdv;
  ...
  case 'A':
    rect_foreach(rect, i, j) {
      color_rgb2gray(img->ie[rect->r + i][rect->c + j].r,
                     img->ie[rect->r + i][rect->c + j].g,
                     img->ie[rect->r + i][rect->c + j].b, &n);
      davg += n;
      dstdv += n * n;
    }
    break;
...
IMAGE *image_hmerge(IMAGE *image1, IMAGE *image2) {
  ...
  image = image_create(MAX(image1->height, image2->height),
...
// matter center
int image_rect_mcenter(IMAGE *img, RECT *rect, char orgb, int *crow,
                       int *ccol) {
  BYTE n;
  int i, j;
...
#else // Fast
  for (i = 0; i < rect->h; i++) {
    memcpy(&(sub->ie[i][0]), &(img->ie[i + rect->r][rect->c]),
           sizeof(RGBA_8888) * rect->w);
  }
#endif
  return sub;
}
...
MATRIX *image_gstatics(IMAGE *img, int rows, int cols) {
  ...
  ball = bh * bw;
  matrix_foreach(mat, i, j) mat->me[i][j] /= ball;
    ...
    for (j = 0; j <= grid_cols; j++) {
      ...
      for (i2 = rect.r; i2 < rect.r + rect.h && i2 < image->height; i2++) {
        ...
        for (j2 = rect.c; j2 < rect.c + rect.w && j2 < image->width; j2++) {
          ...
          // R
          g = image->ie[i2][j2].r;
          d = (1.0 - u) * (1.0 - v) * d1->map[g] + (1.0 - u) * v * d2->map[g] +
              u * (1.0 - v) * d3->map[g] + u * v * d4->map[g];
          g = (d > 255) ? 255 : (int)d;
          image->ie[i2][j2].r = g;
...
int image_niblack(IMAGE *image, int radius, float scale) {
  ...
  matrix_foreach(area, i, j) {
    area->me[i][j] =
        matrix_difference(temp, i - radius, j - radius, i + radius, j + radius);
  }
  matrix_destroy(temp);
  ...
  // stdv
  matrix_foreach(stdv, i, j) stdv->me[i][j] = stdv->me[i][j] * stdv->me[i][j];
