        ...
        for (i = 0; i < (rect)->h; i++) {
            ...
            for (j = 0; j < (rect)->w; j++) {
                if (j + rect->c < 0 || j + rect->c >= img->width)
                    continue;
                y2 = (66 * (img->ie[i + rect->r][j + rect->c].r) + 129 * (img->ie[i + rect->r][j + rect->c].g) + 25 * (img->ie[i + rect->r][j + rect->c].b) + 128) / 256 + 16;
...
{
    return (!img || img->height < 1 || img->width < 1 || !img->ie || img->magic != IMAGE_MAGIC)
        ? 0
        : 1;
}
